...
handlers._users.post = function(data, callback) {
  ...
  var phone =
    checkIsString(data.payload.phone) && data.payload.phone.trim().length > 10
      ...
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          _data.create("tokens", tokenId, tokenObject, function(err) {
            if (!err) {
              tokenObject[acceptebleMethods] = {
                changeInformation:
                  "You can change your personal info like a email, firstName, lastName or street address",
                getInformation: "You could get all your information",
                delete: "You can delete your account"
              }
...
handlers._tokens.delete = function(data, callback) {
  // Check that the id is valid
  var id = checkIsString(data.queryStringObject.id) && data.queryStringObject.id.trim().length == 20 ? data.queryStringObject.id.trim() : false;
  if (id) {
    // Lookup the id
...
handlers._checks.post = function(data, callback) {
  // validate inputs
  var protocol =
    checkIsString(data.payload.protocol) && ~["https", "http"].indexOf(data.payload.protocol) ? data.payload.protocol : false;
  var url = checkIsString(data.payload.url) && data.payload.url.trim().length > 0 ? data.payload.url : false;
  var method = checkIsString(data.payload.method) && ~["post", "get", "put", "delete"].indexOf(data.payload.method) ? data.payload.method : false;
